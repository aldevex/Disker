...
enum class SizeSig
{
    ...
    InvalidNumber = SIZE_MAX, // Failed to convert the raw number
    NoUnit = (SIZE_MAX -1), // No unit after number when expected
    InvalidUnit = (SIZE_MAX -2), // Invalid unit after number (e.g. "32volts")
    TooBigResult = (SIZE_MAX -3), // The result is above SIZE_MAX and/or is inside this enum
    
    ENUM_FIRST = TooBigResult,
};

constexpr size_t CONVSIZE_MAX = (size_t)SizeSig::ENUM_FIRST -1;

// Converts strings to unsigned integer with multiplying unit, e.g. "400", "32gib", "6mb", "40b"
// May return a specific enum value in the size_t on failure (check with SizeSig enum)
size_t strToSize(std::string_view view, bool unitExpected, bool alwaysBinaryUnits)
{
    ...
    char* endPtr = nullptr;
    size_t rawNum = strtoull(copy.c_str(), &endPtr, 0);
    // Total failure
    if (endPtr == nullptr) return (size_t)SizeSig::InvalidNumber;
    // Unexpected unit (or random bs text stuck after raw number)
    else if (!unitExpected && endPtr != '\0') return (size_t)SizeSig::InvalidNumber;
    // No unit
    else if (unitExpected && endPtr == '\0') return (size_t)SizeSig::NoUnit;

    // Convert according to unit
    static auto multiply = [&](size_t multiplier, bool shiftNotMultiply) -> size_t
    {
        // Check if 1. raw number itself is inside enum
        //   or 2. conversion will overflow size_t and/or enum limit (both checks work with only CONVSIZE_MAX)
        ...
        if ( (shiftNotMultiply && rawNum > CONVSIZE_MAX / (1ULL << multiplier))
          || (!shiftNotMultiply && rawNum > CONVSIZE_MAX / (multiplier)) )
            return (size_t)SizeSig::TooBigResult;
    };
    ...
    return (size_t)SizeSig::InvalidUnit;
}

// Returns read buffer and (true on error) bool
std::pair<std::string, bool> readFile(const char* path)
{
